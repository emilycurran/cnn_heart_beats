...
def select_data(database_path, num_samples = 1000):
...
def fetch_selected_data(database_path):
    conn = sqlite3.connect(database_path)
...
def split_data(database_path, train_split=0.8, seed=42):
    """split the data deterministically"""
    
    # fetch the data
    img_paths , time_series, labels = fetch_selected_data(database_path)
    if not img_paths:
