...
def get_record_paths(): 
    paths = glob.glob(os.path.join(parent_dir, "mit-bih-arrhythmia-database-1.0.0/*.atr"))
...
def generate_augmented_samples(database_path, target_samples = 1000):
...
def create_and_save_plots(database_name, output_base_dir):
    ...
    os.makedirs(os.path.join(parent_dir, output_base_dir), exist_ok=True)    
    ...
    cursor.execute("""SELECT record_number, arrhythmia_type, occurrence, augmentation_no, time_series
                    FROM ecg_table
                    WHERE image_file_path IS NULL; """) #don't bother running on already generated images
    rows = cursor.fetchall()
    ...
    for record_number, arrhythmia_type, occurrence, augmentation_no, time_series in rows:        
        ...
        arrhythmia_dir = os.path.join(os.path.join(parent_dir, output_base_dir), safe_label)
...
def main(): 
    ...
    print(database_path)
    save_to_sqlite(database_path, get_record_paths(), labels)
    generate_augmented_samples(database_path)
    create_and_save_plots(database_path, "plots")
    utils.select_data(database_path) #create a table containin only the desired data
